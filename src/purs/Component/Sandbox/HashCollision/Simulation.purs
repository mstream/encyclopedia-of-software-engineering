...
render { algorithm, input1, input2 } =
  ...
  where
  collisionRatio = (Int.toNumber $ 100 * numberOfCollidingBits) / (Int.toNumber totalNumberOfBits)
  ...
  collisionMask = List.zipWith
    ...
    inputDigestBits1
    inputDigestBits2
  inputDigestBits1 = Binary.toBits <$> HashCollision.digest algorithm input1
  inputDigestBits2 = Binary.toBits <$> HashCollision.digest algorithm input2
...
renderDigest =
  HH.div [ classes [ "flex", "flex-row", "flex-wrap", "font-mono" ] ]
    <<< Array.fromFoldable
    <<< map renderByte
